...
export function useArticleFilter(): ArticleFilterState & ArticleFilterActions {
  ...
  const [selectedDomain, setSelectedDomainRaw] = useState("");
  ...
  // filter/sort/category/domain setter에서 직접 page 리셋 (React 18 batching으로 단일 렌더)
  ...
  const setSelectedDomain = useCallback((value: string) => {
    setSelectedDomainRaw(value);
    setPage(1);
  }, []);
